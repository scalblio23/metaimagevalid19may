...
async function createCampaign(
  ...
  const payload: any = {
    ...
    objective,
    status,
    special_ad_categories: JSON.stringify(options.specialAdCategories || []),
    access_token: META_ACCESS_TOKEN,
  };
  ...
  if (options.bidStrategy) payload.bid_strategy = options.bidStrategy;

  const response = await axios.post(`${GRAPH_API_BASE}/${adAccountId}/campaigns`, payload);
  return { id: response.data.id, name };
}
...
async function createAdSet(
  ...
  if (options.dailyBudget !== undefined) payload.daily_budget = options.dailyBudget;
  if (options.lifetimeBudget !== undefined) payload.lifetime_budget = options.lifetimeBudget;
  if (options.bidAmount !== undefined) payload.bid_amount = options.bidAmount;
  if (options.bidStrategy) payload.bid_strategy = options.bidStrategy;
  if (options.startTime) payload.start_time = options.startTime;
  if (options.endTime) payload.end_time = options.endTime;
  if (options.promotedObject) payload.promoted_object = JSON.stringify(options.promotedObject);

  const response = await axios.post(`${GRAPH_API_BASE}/${adAccountId}/adsets`, payload);
  return { id: response.data.id, name };
}
...
function createMcpServer(): McpServer {
  ...
  server.tool(
    "create_campaign",
    "Create a new campaign in a Meta Ad Account. A campaign defines the advertising objective. Ad sets and ads live inside it. Returns the campaign ID needed to create ad sets.",
    {
      ad_account_id: z
        .string()
        .describe("Meta Ad Account ID (format: act_XXXXXXXXX)."),
      name: z
        .string()
        .describe("Name for the campaign (internal label)."),
      objective: z
        .string()
        .describe("Campaign objective. Common values: OUTCOME_TRAFFIC, OUTCOME_SALES, OUTCOME_LEADS, OUTCOME_ENGAGEMENT, OUTCOME_AWARENESS, OUTCOME_APP_PROMOTION."),
      status: z
        .string()
        .optional()
        .describe("Campaign status: ACTIVE or PAUSED (default: PAUSED)."),
      special_ad_categories: z
        .array(z.string())
        .optional()
        .describe("Special ad categories if applicable: HOUSING, EMPLOYMENT, CREDIT, ISSUES_ELECTIONS_POLITICS, FINANCIAL_PRODUCTS_SERVICES. Default: none (empty array)."),
      daily_budget: z
        .number()
        .optional()
        .describe("Daily budget in the account currency's minor units (e.g., cents). Sets a campaign-level budget (CBO). Omit to set budget at the ad set level instead."),
      lifetime_budget: z
        .number()
        .optional()
        .describe("Lifetime budget in minor units (e.g., cents). Campaign-level budget (CBO). Mutually exclusive with daily_budget."),
      bid_strategy: z
        .string()
        .optional()
        .describe("Bid strategy: LOWEST_COST_WITHOUT_CAP, LOWEST_COST_WITH_BID_CAP, COST_CAP. Required when setting a campaign-level budget."),
    },
    async ({ ad_account_id, name, objective, status, special_ad_categories, daily_budget, lifetime_budget, bid_strategy }) => {
      try {
        if (!META_ACCESS_TOKEN) {
          return { content: [{ type: "text" as const, text: "Error: META_ACCESS_TOKEN is not configured on the server." }] };
        }

        const result = await createCampaign(ad_account_id, name, objective, status || "PAUSED", {
        ...
        return {
          content: [{
            ...
            text: JSON.stringify({
              ...
              message: `Campaign created successfully. Campaign ID: "${result.id}". Use this ID with create_adset to add ad sets.`,
            }, null, 2),
          }],
        };
      } catch (error: any) {
  ...
  server.tool(
    "create_adset",
    "Create a new ad set within a campaign. An ad set controls budget, schedule, optimization, bidding, and audience targeting. Returns the ad set ID needed to create ads.",
    {
      ad_account_id: z
        .string()
        .describe("Meta Ad Account ID (format: act_XXXXXXXXX)."),
      name: z
        .string()
        .describe("Name for the ad set (internal label)."),
      campaign_id: z
        .string()
        .describe("The campaign ID this ad set belongs to (from create_campaign or list_campaigns)."),
      optimization_goal: z
        .string()
        .describe("What to optimize delivery for. Common values: LINK_CLICKS, LANDING_PAGE_VIEWS, OFFSITE_CONVERSIONS, REACH, IMPRESSIONS, LEAD_GENERATION, THRUPLAY, POST_ENGAGEMENT."),
      billing_event: z
        .string()
        .describe("What you get billed for. Common values: IMPRESSIONS, LINK_CLICKS, THRUPLAY."),
      targeting: z
        .string()
        .describe('JSON string of the targeting spec. Example: {"geo_locations":{"countries":["US"]},"age_min":18,"age_max":65}'),
      status: z
        .string()
        .optional()
        .describe("Ad set status: ACTIVE or PAUSED (default: PAUSED)."),
      daily_budget: z
        .number()
        .optional()
        .describe("Daily budget in minor units (e.g., cents). Required unless the campaign uses a campaign-level budget (CBO). Mutually exclusive with lifetime_budget."),
      lifetime_budget: z
        .number()
        .optional()
        .describe("Lifetime budget in minor units (e.g., cents). Requires end_time. Mutually exclusive with daily_budget."),
      bid_amount: z
        .number()
        .optional()
        .describe("Bid amount in minor units (e.g., cents). Required for bid strategies with a cap."),
      bid_strategy: z
        .string()
        .optional()
        .describe("Bid strategy: LOWEST_COST_WITHOUT_CAP, LOWEST_COST_WITH_BID_CAP, COST_CAP. Only set here if not set at the campaign level."),
      start_time: z
        .string()
        .optional()
        .describe("Start time in ISO 8601 format (e.g., 2026-06-01T00:00:00-0700)."),
      end_time: z
        .string()
        .optional()
        .describe("End time in ISO 8601 format. Required when using lifetime_budget."),
      promoted_object: z
        .string()
        .optional()
        .describe('JSON string of the promoted object (required for some optimization goals like OFFSITE_CONVERSIONS). Example: {"pixel_id":"123","custom_event_type":"PURCHASE"}'),
    },
    async ({ ad_account_id, name, campaign_id, optimization_goal, billing_event, targeting, status, daily_budget, lifetime_budget, bid_amount, bid_strategy, start_time, end_time, promoted_object }) => {
      try {
        if (!META_ACCESS_TOKEN) {
          return { content: [{ type: "text" as const, text: "Error: META_ACCESS_TOKEN is not configured on the server." }] };
        }

        let parsedTargeting: any;
        try {
          parsedTargeting = JSON.parse(targeting);
        } catch (e) {
          return { content: [{ type: "text" as const, text: "Error: targeting must be valid JSON." }] };
        }

        let parsedPromotedObject: any;
        if (promoted_object) {
          try {
            parsedPromotedObject = JSON.parse(promoted_object);
          } catch (e) {
            return { content: [{ type: "text" as const, text: "Error: promoted_object must be valid JSON." }] };
          }
        }
        ...
        const result = await createAdSet(
          ...
          status || "PAUSED",
          {
            ...
            endTime: end_time,
            promotedObject: parsedPromotedObject,
          }
        );
        ...
        return {
          content: [{
            ...
            text: JSON.stringify({
              ...
              name: result.name,
              status: status || "PAUSED",
              message: `Ad set created successfully. Ad set ID: "${result.id}". Use this ID with create_ad to add ads.`,
            }, null, 2),
          }],
        };
      } catch (error: any) {
...
app.listen(PORT, "0.0.0.0", () => {
  ...
  console.log(`Tools: 19 (including 4 insights/metrics tools)`);
});
