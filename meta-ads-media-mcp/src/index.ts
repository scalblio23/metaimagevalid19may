...
function validateImageMime(mimeType: string, fileName: string): void {
  ...
  if (!validMimes.includes(mimeType) && !validExts.includes(ext)) {
    throw new Error(
      `Invalid image format. MIME: "${mimeType}", extension: "${ext}". Supported: jpg, png, gif, bmp, tiff, webp`
    );
  }
}

function validateVideoMime(mimeType: string, fileName: string): void {
  ...
  if (!validMimes.includes(mimeType) && !validExts.includes(ext)) {
    throw new Error(
      `Invalid video format. MIME: "${mimeType}", extension: "${ext}". Supported: mp4, mov, avi, mkv, webm, gif`
    );
  }
}
